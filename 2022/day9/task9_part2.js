...
for (let c of commands) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    followHead(H, points[0]);

    for (let k = 1; k < points.length; k++) {
      followHead(points[k - 1], points[k]);
    }
...
console.log("part 2 answer", visited.size);

function visualizePath(visited, symbol = "#") {
  let minx = 0;
  let miny = 0;
  let maxx = 0;
  let maxy = 0;
